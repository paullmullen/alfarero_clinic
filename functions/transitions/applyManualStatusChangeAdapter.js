...
function reconcileEncounterForWaiting({
  stationEntry,
  previousStatus,
  timestamp,
  source,
  ...
  // Fresh waiting state starts/restarts a waiting window.
  encounter.waiting_start = timestamp || encounter.waiting_start || null;
  encounter.waiting_end = null;
  encounter.waiting_time = null;

  if (previousStatus === "complete") {
    encounter.in_process_start = null;
...
function reconcileEncounterForWaitingExit({ stationEntry, timestamp, source }) {
  ensureEncountersArray(stationEntry);

  const encounter = getOpenEncounter(stationEntry);
  if (!encounter) {
    recomputeStationTotals(stationEntry);
    return null;
  }

  // Close a waiting-only encounter when manual flow demotes it
  // back to planned/pending or otherwise abandons waiting.
  if (!encounter.in_process_start) {
    encounter.waiting_start =
      encounter.waiting_start || stationEntry.waiting_start || timestamp;
    encounter.waiting_end = stationEntry.waiting_end || timestamp || null;
    encounter.waiting_time = diffSeconds(
      encounter.waiting_start,
      encounter.waiting_end,
    );
...
export function applyManualStatusChangeAdapter({
  visitData,
  station,
  newStatus,
  timestamp,
  ...
  const enginePlan = applyManualStatusChange(
    originalPlan,
    station,
    newStatus,
    timestamp,
  ...
  for (const updatedStep of updatedPlan) {
    ...
    if (previousStatus === nextStatus) {
      recomputeStationTotals(updatedStep);
      continue;
    }

    if (nextStatus === "waiting") {
      reconcileEncounterForWaiting({
    ...
    if (previousStatus === "waiting" && nextStatus !== "waiting") {
      reconcileEncounterForWaitingExit({
        stationEntry: updatedStep,
        timestamp,
        source,
      });
